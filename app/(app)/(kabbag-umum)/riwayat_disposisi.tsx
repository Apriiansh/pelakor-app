...
type StatusFilter = 'semua' | 'diproses' | 'ditolak' | 'ditindaklanjuti' | 'selesai';

export default function RiwayatDisposisiScreen(): JSX.Element {
    ...
    const fetchLaporan = useCallback(async (): Promise<void> => {
        try {
            ...
            const historyLaporan = data.filter(
                (l: Laporan) => l.status_laporan !== 'diajukan'
            );
            setLaporanList(historyLaporan);
        } catch (error) {
            const errorMessage = error instanceof api.ApiError ? error.message : 'Gagal memuat riwayat disposisi.';
    ...
    const openDetail = useCallback(async (laporan: Laporan): Promise<void> => {
        ...
        try {
            ...
            console.log('Disposisi History received:', disposisi); // Added log
    ...
    const renderEmptyState = useCallback(() => (
        <View style={styles.centered}>
            <Text variant="headlineSmall" style={[styles.emptyTitle, { color: theme.colors.onSurfaceVariant }]}>
                {searchQuery || statusFilter !== 'semua' ? 'Tidak ada disposisi ditemukan' : 'Belum ada riwayat disposisi'}
            </Text>
            <Text style={[styles.emptySubtitle, { color: theme.colors.onSurfaceVariant }]}>
                {searchQuery || statusFilter !== 'semua' ? 'Coba ubah filter atau kata kunci pencarian' : 'Riwayat disposisi akan muncul di sini setelah Anda memproses laporan.'}
            </Text>
        </View>
    ...
    if (loading) {
        return (
            <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
                <Stack.Screen options={{ title: 'Riwayat Disposisi' }} />
                <Appbar.Header>
                    <Appbar.BackAction onPress={() => router.back()} />
                    <Appbar.Content title="Riwayat Disposisi" titleStyle={styles.appbarTitle} />
                </Appbar.Header>
                <View style={styles.centered}>
            ...
            {(searchQuery || statusFilter !== 'semua') && (
                <View style={styles.resultContainer}>
                    <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }}>
                        Ditemukan {filteredData.length} disposisi
                    </Text>
                    <Button
                        mode="text"
                        onPress={() => {
